...
>
> `StatType.SPEED` はパーセント値で定義します。たとえば `5.0` は移動速度 `+5%`、`-10.0` は `-10%` として計算・表示されます。

---

## 4. 装備の耐久値

武器種が設定されたカスタムアイテム、防具、`PickaxeItem` は耐久システムの対象です。
最大耐久値はレアリティと装備種別から自動決定され、ツルハシは
`PickaxeItem#getToolDurability()` の値を優先します。

個別の最大耐久値が必要な場合は `getMaxDurability()` をオーバーライドします。
`-1` は自動決定、`0` は耐久対象外を表します。
...
耐久値が尽きた装備は消滅せず破損状態になり、武器としての使用と装備効果が停止します。
破損・損傷した装備は合成屋の「装備修理」から修理できます。
ランダムステータス、特殊効果、セット情報は修理後も維持されます。
