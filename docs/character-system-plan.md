...
* 既存プレイヤーにアクティブな選択可能キャラクターがない場合、別キャラを自動選択せずキャラクター選択GUIを開く。
* ソフトHC/真HC死亡時はイベント時点のアクティブキャラクターをsnapshotし、永続化処理は非同期で行う。キャラクター選択GUIは死亡直後ではなく保存完了後かつリスポーン後に開く。
* 死亡snapshotの保存が完了するまで、そのプレイヤーのキャラクター作成/選択はロックする。
